...
function bidiCounter() {
    ...
    return function(mesg) {
        return (mesg === 'inc') ? ++x : (mesg === 'dec') ? --x : x;
    };
}
...
function point(x) {
    let ordX = 0;
    return function(mesg, el) {
        if (mesg === "x") return ordX;
        else if (mesg === "equals" && !!el) return ordX === el('x');
        else if (mesg === "setX" && !isNaN(el)) ordX = el;
...
function object(state, methods) {
    return function() {
        if (arguments.length > 0) {
            if (Object.keys(methods).indexOf(arguments[0]) >= 0) {
                return methods[arguments[0]](state);
            }
        }
        return "Method not understood";
    }
}
